...
def iniciar_recorrido():
    ...
    open_recorrido_chofer = RecorridoModel.query.filter_by(chofer_id=chofer_id, estado='abierto').first()
    if open_recorrido_chofer:
        return jsonify({"msg": "El chofer ya tiene un recorrido abierto"}), 409

    open_recorrido_vehiculo = RecorridoModel.query.filter_by(vehiculo_id=data.vehiculo_id, estado='abierto').first()
...
def fin_recorrido(recorrido_id):
    ...
    recorrido = RecorridoModel.query.filter_by(id=recorrido_id, chofer_id=chofer_id, estado='abierto').first()
    if not recorrido:
...
def detalle_recorrido(recorrido_id):
    ...
    recorrido = RecorridoModel.query.get_or_404(recorrido_id)
    return jsonify(RecorridoSchema.model_validate(recorrido).model_dump()), 200
